...
const Shop = () => {
  ...
  useEffect(() => {
    const url = `allitems?${new URLSearchParams(searchParams).toString()}`;
    axiosSecure
      .get(url)
  ...
  const handleFilterClick = (type, value) => {
    ...
    if (type === 'sort') {
      ...
      if (newParams.get('category') === value) newParams.delete('category');
      else {
        newParams.set('category', value);
        newParams.delete('shape');
      }
    } else if (type === 'shape') {
      if (newParams.get('shape') === value) newParams.delete('shape');
      else {
      ...
      <div className="drawer">
        ...
        <div className="drawer-side">
          ...
          <ul className="menu bg-base-200 text-base-content min-h-full w-80 p-4">
            ...
            <li>
              <button
                onClick={() => handleFilterClick('sort', 'low_to_high')}
                className={`w-full text-left ${searchParams.get('sort') === 'low_to_high' ? 'font-bold text-[#329c92]' : ''}`}
              >
                Price: Low to High
              </button>
            </li>
            <li>
              <button
                onClick={() => handleFilterClick('sort', 'high_to_low')}
                className={`w-full text-left ${searchParams.get('sort') === 'high_to_low' ? 'font-bold text-[#329c92]' : ''}`}
              >
                Price: High to Low
              </button>
            </li>
